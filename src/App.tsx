...
interface LiveRepo extends Repo { live?: boolean }

interface TrendingRepo {
  name: string; desc: string; stars: number; forks: number
  lang: string; owner: string; topics: string
  created_at: string; pushed_at: string; trending: boolean
}
...
export default function App() {
  ...
  // Trending names set for globe
  const trendingNames = useMemo(() =>
    new Set(trendingRepos.map(r=>r.name)), [trendingRepos])

  // Keyboard shortcuts
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key==='/' && document.activeElement!==searchRef.current) { e.preventDefault(); searchRef.current?.focus() }
      if (e.key==='Escape') { setShowResults(false);setShowFilters(false);setShowAbout(false);setShowTrending(false);setSelected(null);searchRef.current?.blur() }
  ...
  const filteredRepos = useMemo(()=>{
    ...
    return base.map(r => trendingMode && trendingNames.has(r.name)
      ? { ...r, trending: true }
      : r
    )
  }, [repos, langFilter, regionFilter, starsFilter, trendingMode, trendingNames])
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if(e.key==='Enter'&&activeIdx>=0){const all=[...localResults,...liveResults];if(all[activeIdx])selectRepo(all[activeIdx])}
  ...
  return (
    <>
      ...
      {!loading && (
        <Globe repos={filteredRepos} onSelect={setSelected} onHover={handleHover} flyTarget={flyTarget} trendingNames={trendingNames}/>
      )}
      ...
      <div className="hud-top">
        ...
        <div className="search-wrap" onBlur={()=>setTimeout(()=>{setShowResults(false);setShowFilters(false)},180)}>
          ...
          <input ref={searchRef} className="search-input" type="text"
            placeholder="Search any GitHub repo… ( / )"
            value={query} onChange={e=>setQuery(e.target.value)}
            onKeyDown={handleKeyDown} onFocus={()=>query&&setShowResults(true)} autoComplete="off"/>
            ...
            <div className="search-results">
              {localResults.length>0&&(
                ...
                {localResults.map((r,i)=>(
                  <div key={r.name} className={`result-item ${i===activeIdx?'active':''}`} onMouseDown={()=>selectRepo(r)}>
              ...
              <div className="results-section-label">
                Live GitHub search {liveLoading&&<span className="live-spinner"/>}
              </div>
          ...
          {showFilters&&(
            <div className="filter-panel">
                ...
                <div className="chip-row">
                  {STAR_FILTERS.map(s=>(
                    <button key={s.label} className={`chip ${starsFilter===s.min?'active':''}`}
                      onMouseDown={e=>{e.preventDefault();setStarsFilter(starsFilter===s.min?null:s.min)}}>
                      {s.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="chip-row">
                  {Object.keys(REGIONS).map(r=>(
                    <button key={r} className={`chip ${regionFilter===r?'active':''}`}
                      onMouseDown={e=>{e.preventDefault();setRegionFilter(regionFilter===r?null:r)}}>
                      {r}
                    </button>
                  ))}
                </div>
              ...
              {activeFilterCount>0&&(
                <button className="clear-filters" onMouseDown={e=>{e.preventDefault();clearFilters()}}>Clear all filters</button>
              )}
        ...
        {/* Stats + Trending toggle + Share */}
        <div className="hud-stats">
          <div className="stat-pill">
            <span className="stat-dot"/>
            <span className="stat-val">{repoCount}</span>
            <span className="stat-label">repos</span>
          </div>
          <div className="stat-pill">
            <span className="stat-val">{countryCount}</span>
            <span className="stat-label">countries</span>
          </div>

          {/* Trending button */}
          <button
            className={`trending-btn ${trendingMode?'active':''}`}
            onClick={()=>setTrendingMode(v=>!v)}
            title="Toggle trending mode (T)"
          >
          ...
          <button
            className={`trending-list-btn ${showTrending?'active':''}`}
            onClick={()=>setShowTrending(v=>!v)}
            title="Show trending list"
          >
            <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
          ...
          <button className={`share-btn ${copied?'copied':''}`} onClick={handleShare} title="Copy link">
            {copied
      ...
      {showTrending&&(
        <div className="trending-panel">
          ...
          {trendingRepos.length===0&&!trendingLoading&&(
            <div className="trending-loading">No trending data available</div>
          )}
        </div>
      )}

      {/* Active filters bar */}
      {(activeFilterCount>0||trendingMode)&&(
        <div className="active-filters-bar">
          {trendingMode&&(
            <span className="active-chip active-chip-trend">
              🔥 Trending only
              <button onClick={()=>setTrendingMode(false)}>×</button>
            </span>
          )}
          {langFilter&&(
            <span className="active-chip" style={{borderColor:getLangColor(langFilter)}}>
              <span className="chip-dot" style={{background:getLangColor(langFilter)}}/>
              {langFilter}<button onClick={()=>setLangFilter(null)}>×</button>
            </span>
          )}
          {regionFilter&&<span className="active-chip">{regionFilter}<button onClick={()=>setRegionFilter(null)}>×</button></span>}
          {starsFilter&&(
            <span className="active-chip">
              ⭐ {starsFilter>=1000?(starsFilter/1000)+'k+':starsFilter+'+'}
              <button onClick={()=>setStarsFilter(null)}>×</button>
            </span>
          )}
          <button className="clear-all-btn" onClick={()=>{clearFilters();setTrendingMode(false)}}>Clear all</button>
        </div>
      )}
      ...
      <div className={`info-panel ${selected?'open':''}`}>
        <div className="panel-header">
          ...
          {selected&&trendingNames.has(selected.name)&&(
            <span className="trending-flame-badge">🔥 Trending</span>
          )}
          {(selected as LiveRepo)?.live&&<span className="live-badge panel-live-badge">LIVE</span>}
        ...
        {selected&&(
          <div className="panel-body">
            ...
            <div className="panel-row">
              <span className="panel-key">Language</span>
              <span className="lang-badge"><span className="lang-dot" style={{background:getLangColor(selected.lang)}}/>{selected.lang}</span>
            </div>
            ...
            {selected.lat!==0&&selected.lng!==0&&(
              <div className="panel-coords">lat: {selected.lat.toFixed(4)}  lng: {selected.lng.toFixed(4)}</div>
            )}
            <button className="panel-filter-btn" onClick={()=>{setLangFilter(selected.lang);setSelected(null)}}>
              Show all {selected.lang} repos →
            </button>
          </div>
        )}
      </div>
      ...
      {showAbout&&(
        <div className="about-panel">
          ...
          <div className="about-desc">Interactive 3D map of global open-source innovation. Each node is a real GitHub repository placed at its geographic origin.</div>
          <div className="about-stats">
            ...
            <div className="about-stat"><span>{trendingRepos.length}</span>trending</div>
          </div>
          ...
          <div className="about-phase">Phase 2 — Live + Trending Active</div>
        </div>
      )}
      ...
      {!loading&&<div className="hint">Drag to rotate · Scroll to zoom · T = trending</div>}
    </>
  )
}
